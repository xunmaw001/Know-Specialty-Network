          ...
          <form class="layui-form">
            <div class="layui-inline" style="margin-bottom: 10px;">
              ...
              <div class="layui-input-inline">
                <input type="text" name="techanmingcheng" id="techanmingcheng" placeholder="特产名称" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <div class="layui-inline" style="margin-bottom: 10px;">
              <label class="layui-form-label">特产分类</label>
              <div class="layui-input-inline">
                <input type="text" name="techanfenlei" id="techanfenlei" placeholder="特产分类" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
              ...
              <button v-if="isAuth('techanxinxi','新增')" @click="jump('../techanxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
        ...
        <div class="pro-lists-con">
          <ul class="after">
            <li v-for="(item,index) in dataList" v-bind:key="index">
              <a target="_self" :href="'../techanxinxi/detail.jsp?id='+item.id">
                <div class="posi-img">
                  <img :src="item.tupian?item.tupian.split(',')[0]:''" width="385px" height="220px" />
                </div>
                <div class="remark">
                  <h5>{{item.techanmingcheng}}</h5>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#techanmingcheng').val()) {
          param['techanmingcheng'] = jquery('#techanmingcheng').val() ? '%' + jquery('#techanmingcheng').val() + '%' : '';
        }  
        if (jquery('#techanfenlei').val()) {
          param['techanfenlei'] = jquery('#techanfenlei').val() ? '%' + jquery('#techanfenlei').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('techanxinxi/list', 'get', param, function(res) {
